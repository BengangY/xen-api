...
let test_introduce_multiple_bundle_repos () =
  let __context = T.make_test_database () in
  let name_label = "name" in
  let name_label_1 = "name1" in
  let name_description = "description" in
  ...
  let _ =
    Repository.introduce ~__context ~name_label ~name_description ~binary_url:""
      ~source_url:"" ~update:true ~gpgkey_path ~origin
  in
  Alcotest.check_raises "test_introduce_multiple_bundle_repos"
    Api_errors.(Server_error (bundle_repository_exists, []))
    (fun () ->
...
let test =
  ...
  ; ( "test_introduce_multiple_bundle_repos"
    , `Quick
    , test_introduce_multiple_bundle_repos
    )
  ]
