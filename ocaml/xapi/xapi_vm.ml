...
let update_allowed_operations ~__context ~self =
  Helpers.log_exn_continue
    "updating allowed operations of VBDs/VIFs/VDIs/VUSBs in \
...
let set_ha_restart_priority ~__context ~self ~value =
  ...
  if current <> value then (
    ...
    if Db.VM.get_power_state ~__context ~self = `Running then (
      Db.VM.set_ha_always_run ~__context ~self
        ~value:(value = Constants.ha_restart) ;
      debug
        "Setting ha_always_run on vm=%s as %b during VM.set_ha_restart_priority"
        (Ref.string_of self)
        (value = Constants.ha_restart)
    )
  )
...
let set_memory_static_range ~__context ~self ~min ~max =
  ...
  if (not is_control_domain) && power_state <> `Halted then
    failwith
      "assertion_failed: set_memory_static_range should only be called when \
       the VM is Halted" ;
  (* Check the range constraints *)
  let constraints = Vm_memory_constraints.get ~__context ~vm_ref:self in
  let constraints =
    {constraints with Vm_memory_constraints.static_min= min; static_max= max}
  in
  Vm_memory_constraints.assert_valid_for_current_context ~__context ~vm:self
    ~constraints ;
...
let set_memory_limits ~__context ~self ~static_min ~static_max ~dynamic_min
  ...
  if (not is_control_domain) && power_state <> `Halted then
    failwith
      "assertion_failed: set_memory_limits should only be called when the VM \
       is Halted" ;
  (* Check that the new limits are in the correct order. *)
  let constraints =
    {
      Vm_memory_constraints.static_min
    ; dynamic_min
    ; target= dynamic_min
    ; dynamic_max
    ; static_max
...
let assert_not_ha_protected ~__context ~vm =
  ...
  then
    raise
      (Api_errors.Server_error (Api_errors.vm_is_protected, [Ref.string_of vm]))

let pause ~__context ~vm = Xapi_xenops.pause ~__context ~self:vm

let unpause ~__context ~vm = Xapi_xenops.unpause ~__context ~self:vm
...
let assert_memory_constraints ~__context ~vm platformdata =
  ...
  ( if Vm_platform.is_true ~key:"nested-virt" ~platformdata ~default:false then
      (* CP-18860: check memory limits if using nested_virt on start *)
      let c = C.get ~__context ~vm_ref:vm in
      C.assert_valid_and_pinned_at_static_max ~constraints:c
        ~reason:"nested virt"
  ) ;
  let vgpus = Db.VM.get_VGPUs ~__context ~self:vm in
  if List.exists (is_sriov_vgpu ~__context) vgpus then
...
let start ~__context ~vm ~start_paused ~force =
  ...
  if vmr.API.vM_ha_restart_priority = Constants.ha_restart then (
    ...
    debug "Setting ha_always_run on vm=%s as true during VM.start"
      (Ref.string_of vm)
  ) ;
  let pool = Helpers.get_pool ~__context in
  let default_value =
  ...
  let platform =
    ...
    |> Xapi_vm_helpers.ensure_device_model_profile_present ~__context
         ~domain_type:vmr.API.vM_domain_type ~default_value ~is_a_template:false
    |> update_platform_secureboot ~__context ~self:pool
  in
  let vmr =
    if platform <> vmr.API.vM_platform then (
...
let hard_shutdown ~__context ~vm =
  ...
  debug "Setting ha_always_run on vm=%s as false during VM.hard_shutdown"
    (Ref.string_of vm) ;
  ...
  | `Suspended ->
      ...
      if vdi <> Ref.null (* avoid spurious but scary messages *) then
        Helpers.log_exn_continue
...
let clean_reboot ~__context ~vm =
  update_vm_virtual_hardware_platform_version ~__context ~vm ;
  Xapi_xenops.reboot ~__context ~self:vm
    (Some !Xapi_globs.domain_shutdown_total_timeout)

let clean_shutdown_with_timeout ~__context ~vm timeout =
  Db.VM.set_ha_always_run ~__context ~self:vm ~value:false ;
  debug "Setting ha_always_run on vm=%s as false during VM.clean_shutdown"
    (Ref.string_of vm) ;
...
let shutdown ~__context ~vm =
  ...
  with e ->
    warn
      "Failed to perform clean_shutdown on VM:%s due to exception %s. Now \
       attempting hard_shutdown."
...
let power_state_reset ~__context ~vm =
  ...
  if Db.VM.get_is_control_domain ~__context ~self:vm then (
    error "VM.power_state_reset vm=%s blocked because VM is a control domain"
      (Ref.string_of vm) ;
    raise
      (Api_errors.Server_error
  ...
  if Xapi_vm_lifecycle_helpers.is_live ~__context ~self:vm then (
    debug
      "VM.power_state_reset vm=%s power state is either running or paused: \
    ...
    if resident = localhost then (
      ...
      let running =
        try
          ...
          if s.Vm.power_state = Running then (
            debug "VM.power_state_reset vm=%s xenopsd reports running;"
              (Ref.string_of vm) ;
            true
          ) else (
      ...
      debug
        "VM.power_state_reset vm=%s resident_on<>localhost; checking liveness \
         of remote host"
        (Ref.string_of vm) ;
      if Xapi_host.is_host_alive ~__context ~host:resident then (
        error
          "VM.power_state_reset vm=%s resident_on=%s; host is alive so \
           refusing to reset power-state"
...
let resume ~__context ~vm ~start_paused ~force =
  ...
  then (
    Db.VM.set_ha_always_run ~__context ~self:vm ~value:true ;
    debug "Setting ha_always_run on vm=%s as true during VM.resume"
      (Ref.string_of vm)
  ) ;
  let host = Helpers.get_localhost ~__context in
  if not force then
...
let create ~__context ~name_label ~name_description ~power_state ~user_version
  ...
  *)
  ...
  Db.VM.create ~__context ~ref:vm_ref ~uuid:(Uuidx.to_string uuid)
...
let copy ~__context ~vm ~new_name ~sr =
  ...
  let sr =
    try
      ignore (Db.SR.get_uuid ~__context ~self:sr) ;
      Some sr
    with _ -> None
  in
  Option.iter
...
let provision ~__context ~vm =
  ...
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      ...
      if Xapi_templates.needs_to_be_installed rpc session_id vm then (
...
let set_VCPUs_max ~__context ~self ~value =
  ...
  if (not is_control_domain) && power_state <> `Halted then
    failwith
      "assertion_failed: set_VCPUs_max should only be called when the VM is \
       Halted" ;
  let vcpus_at_startup = Db.VM.get_VCPUs_at_startup ~__context ~self in
  if value < 1L || value < vcpus_at_startup then
...
let is_power_of_2 n = n > 1 && n land (0 - n) = n
...
let wait_memory_target_live ~__context ~self =
  ...
  let rec wait accumulated_wait_time_seconds =
    if accumulated_wait_time_seconds > timeout_seconds then
      raise_error Api_errors.vm_memory_target_wait_timeout ;
    if TaskHelper.is_cancelling ~__context then
      raise_error Api_errors.task_cancelled ;
    ...
    let difference_bytes =
      Int64.abs (Int64.sub memory_actual_bytes memory_target_bytes)
    in
    debug
    ...
    if difference_bytes <= tolerance_bytes then
      ...
      Db.VM_metrics.set_memory_actual ~__context ~self:vm_metrics_ref
      ...
      if is_power_of_2 accumulated_wait_time_seconds then
        debug
          "Waited %i second(s) for VM %s to reach its target = %Ld bytes; \
           actual = %Ld bytes."
          accumulated_wait_time_seconds id memory_target_bytes
          memory_actual_bytes ;
...
let set_memory_dynamic_range ~__context ~self ~min ~max =
  ...
  let constraints =
    if power_state = `Running then
      Vm_memory_constraints.get_live ~__context ~vm_ref:self
    else
      Vm_memory_constraints.get ~__context ~vm_ref:self
  in
  let constraints =
    {
      constraints with
      Vm_memory_constraints.dynamic_min= min
    ; target= min
    ; dynamic_max= max
    }
  in
  ...
  if power_state = `Running then
    Xapi_xenops.set_memory_dynamic_range ~__context ~self min max
...
let request_rdp ~__context ~vm ~enabled =
  ...
  in
  if is_feature_ts2_on then
    Xapi_xenops.request_rdp ~__context ~self:vm enabled
  else
    raise Not_found

let request_rdp_on ~__context ~vm = request_rdp ~__context ~vm ~enabled:true

let request_rdp_off ~__context ~vm = request_rdp ~__context ~vm ~enabled:false
...
let run_script ~__context ~vm ~args =
  ...
  List.iter
    (fun a ->
      if not (List.mem_assoc a args) then
        raise
          (Api_errors.Server_error
  ...
  let session_id =
    Xapi_session.get_top ~__context ~self:(Context.get_session_id __context)
  in
  if
    not
...
let call_plugin ~__context ~vm ~plugin ~fn ~args =
  ...
  try
    ...
    | _ ->
        let msg =
          Printf.sprintf
            "The requested fn \"%s\" could not be found in plugin \"%s\"." fn
            plugin
        in
        raise
          (Api_errors.Server_error
  ...
  with Not_found ->
    let msg =
      Printf.sprintf
        "The requested fn \"%s\" of plugin \"%s\" could not be executed for \
         lack of guest agent control feature."
        fn plugin
    in
    raise
      (Api_errors.Server_error
...
let maximise_memory ~__context ~self ~total ~approximate =
  ...
  let will_fit static_max =
    ...
    let normal, shadow =
      Memory_check.vm_compute_start_memory ~__context
        ~policy:Memory_check.Static_max r
    in
...
let set_snapshot_schedule ~__context ~self ~value =
  ...
  if not (is_vmss_valid_ref || value = Ref.null || Ref.string_of value = "")
  then
    raise
      (Api_errors.Server_error (Api_errors.invalid_value, [Ref.string_of value])) ;
  if value <> Ref.null && is_vmss_valid_ref then (
...
let set_start_delay ~__context ~self ~value =
  if value < 0L then
    invalid_value "start_delay must be non-negative" (Int64.to_string value) ;
  Db.VM.set_start_delay ~__context ~self ~value

let set_shutdown_delay ~__context ~self ~value =
  if value < 0L then
    invalid_value "shutdown_delay must be non-negative" (Int64.to_string value) ;
  Db.VM.set_shutdown_delay ~__context ~self ~value

let set_order ~__context ~self ~value =
  if value < 0L then
    invalid_value "order must be non-negative" (Int64.to_string value) ;
...
let set_suspend_VDI ~__context ~self ~value =
  Xapi_vm_lifecycle.assert_initial_power_state_is ~__context ~self
    ~expected:`Suspended ;
  if Db.VDI.get_type ~__context ~self:value = `cbt_metadata then (
    error "VM.set_suspend_VDI: the given VDI has type cbt_metadata (at %s)"
      __LOC__ ;
    raise
      Api_errors.(
        Server_error
...
let import_convert ~__context ~_type ~username ~password ~sr ~remote_config =
  ...
  let rec loop call vpx_ip =
    ...
    | Created | Queued | Running ->
        Thread.delay 1. ; loop call vpx_ip
    | Completed | Aborted | UserAborted ->
        ()
  in
  debug "import_convert %s"
    (String.concat "; " (List.map (fun (k, v) -> k ^ "," ^ v) remote_config)) ;
  let vpx_ip =
  ...
  let xen_servicecred = {username; password} in
  ...
  let vmware_serverinfo =
    ...
    ; cred=
        {
          username= List.assoc "username" remote_config
        ; password= List.assoc "password" remote_config
        }
    }
  in
  let jobInfo =
    {
      source= vmware_serverinfo
    ; sourceVmUUID= ""
    ; sourceVmName= List.assoc "vm-name" remote_config
    ; importInfo
    }
  in
      ...
      Open_uri.with_open_uri uri (fun fd ->
          ...
          | `Ok r -> (
            ...
            | `OK ->
                ...
                Import.stream_import __context rpc session_id fd None
                  (fun () -> ())
                  Import.{import_type= Full_import sr; full_restore; force}
            | e when Cohttp.Code.is_redirection (Cohttp.Code.code_of_status e)
              -> (
              match
                Cohttp.Header.get (Cohttp.Response.headers r) "Location"
              with
              | Some l ->
                  raise (Retry l)
              | None ->
