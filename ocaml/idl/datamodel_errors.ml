...
let error name params ?(doc = "") () =
  Hashtbl.add errors name {err_name= name; err_params= params; err_doc= doc}

let message name ?(doc = "") () =
  Hashtbl.add messages name {mess_name= name; mess_doc= doc}
...
let _ =
  ...
  error Api_errors.license_processing_error []
    ~doc:
      "There was an error processing your license. Please contact your support \
       representative."
    () ;
  error Api_errors.license_restriction ["feature"]
    ~doc:
      "This operation is not allowed because your license lacks a needed \
       feature. Please contact your support representative."
    () ;
  error Api_errors.license_cannot_downgrade_in_pool []
    ~doc:
      "Cannot downgrade license while in pool. Please disband the pool first, \
       then downgrade licenses on hosts separately."
    () ;
  error Api_errors.license_does_not_support_pooling []
    ~doc:
      "This server cannot join a pool because its license does not support \
       pooling."
    () ;
  ...
  error Api_errors.license_file_deprecated []
    ~doc:
      "This type of license file is for previous versions of the server. \
       Please upgrade to the new licensing system."
    () ;
  ...
  error Api_errors.cannot_contact_host ["host"]
    ~doc:
      "Cannot forward messages because the server cannot be contacted. The \
       server may be switched off or there may be network connectivity \
       problems."
    () ;

  error Api_errors.tls_connection_failed ["address"; "port"]
    ~doc:
      "Cannot contact the other host using TLS on the specified address and \
       port"
    () ;
  ...
  error Api_errors.map_duplicate_key
    ["type"; "param_name"; "uuid"; "key"]
    ~doc:
      "You tried to add a key-value pair to a map, but that key is already \
       there."
    () ;
  error Api_errors.xmlrpc_unmarshal_failure ["expected"; "received"]
    ~doc:
      "The server failed to unmarshal the XMLRPC message; it was expecting one \
       element and received something else."
    () ;
  ...
  error Api_errors.message_method_unknown ["method"]
    ~doc:
      "You tried to call a method that does not exist. The method name that \
       you used is echoed."
    () ;
  error Api_errors.message_parameter_count_mismatch
    ["method"; "expected"; "received"]
    ~doc:
      "You tried to call a method with the incorrect number of parameters. The \
       fully-qualified method name that you used, and the number of received \
       and expected parameters are returned."
    () ;
  error Api_errors.value_not_supported
    ["field"; "value"; "reason"]
    ~doc:
  ...
  error Api_errors.operation_blocked ["ref"; "code"]
    ~doc:
      "You attempted an operation that was explicitly blocked (see the \
       blocked_operations field of the given object)."
    () ;
  ...
  error Api_errors.handle_invalid ["class"; "handle"]
    ...
    () ;
  error Api_errors.db_uniqueness_constraint_violation
    ["table"; "field"; "value"]
    ~doc:
      "You attempted an operation which would have resulted in duplicate keys \
       in the database."
    () ;
  ...
  error Api_errors.session_authorization_failed ["username"; "msg"]
    ~doc:
      "The credentials given by the user are correct, but the user could not \
       be authorized, so access has been denied, and you have not been issued \
       a session handle."
    () ;
  error Api_errors.session_invalid ["handle"]
    ~doc:
      "You gave an invalid session reference. It may have been invalidated by \
       ...
       the current connection. The handle parameter echoes the bad value \
       given."
    () ;
  error Api_errors.change_password_rejected ["msg"]
    ~doc:
      "The system rejected the password change request; perhaps the new \
  ...
  error Api_errors.network_has_incompatible_vlan_on_sriov_pifs
    ["PIF"; "network"]
    ~doc:
  ...
  error Api_errors.network_already_connected
    ["network"; "connected PIF"]
    ~doc:
  ...
  error Api_errors.network_incompatible_purposes
    ["new_purpose"; "conflicting_purpose"]
    ~doc:
      "You tried to add a purpose to a network but the new purpose is not \
       compatible with an existing purpose of the network or other networks."
    () ;
  error Api_errors.pif_is_physical ["PIF"]
    ~doc:
      "You tried to destroy a PIF, but it represents an aspect of the physical \
       host configuration, and so cannot be destroyed. The parameter echoes \
       the PIF handle you gave."
    () ;
  error Api_errors.pif_is_not_physical ["PIF"]
    ~doc:
      "You tried to perform an operation which is only available on physical \
       PIF"
    () ;
  error Api_errors.pif_is_vlan ["PIF"]
    ~doc:
      "You tried to create a VLAN on top of another VLAN - use the underlying \
       physical PIF/bond instead"
    () ;
  error Api_errors.pif_is_sriov_logical ["PIF"]
    ~doc:
      "You tried to create a bond on top of a network SR-IOV logical PIF - use \
       the underlying physical PIF instead"
    () ;
  ...
  error Api_errors.invalid_cidr_address_specified ["parameter"]
    ~doc:
      "A required parameter contained an invalid CIDR address (<addr>/<prefix \
       length>)"
    () ;
  ...
  error Api_errors.cannot_change_pif_properties ["PIF"]
    ~doc:
      "The properties of this PIF cannot be changed. Only the properties of \
       non-bonded physical PIFs, or bond interfaces can be changed."
    () ;
  ...
  error Api_errors.slave_requires_management_iface []
    ~doc:
      "The management interface on a supporter cannot be disabled because the \
       supporter would enter emergency mode."
    () ;
  error Api_errors.vif_in_use ["network"; "VIF"] ~doc:"Network has active VIFs"
    () ;
  error Api_errors.cannot_plug_vif ["VIF"] ~doc:"Cannot plug VIF" () ;
  ...
  error
    ...
    ["device"; "mac"]
    ~doc:
      "Could not find a network interface with the specified device name and \
       MAC address."
    () ;
  ...
  error Api_errors.device_not_attached ["VBD"]
    ~doc:
      "The operation could not be performed because the VBD was not connected \
       to the VM."
    () ;
  ...
  error Api_errors.vgpu_type_not_compatible_with_running_type
    ...
    ~doc:
      "The VGPU type is incompatible with one or more of the VGPU types \
       currently running on this PGPU"
    () ;
  error Api_errors.vgpu_type_not_compatible ["type"]
    ~doc:
      "Cannot create a virtual GPU that is incompatible with the existing \
       types on the VM."
    () ;
  ...
  error Api_errors.is_tunnel_access_pif ["PIF"]
    ~doc:
      "Cannot create a VLAN or tunnel on top of a tunnel access PIF - use the \
       underlying transport PIF instead."
    () ;
  ...
  error Api_errors.vm_is_protected ["vm"]
    ~doc:
      "This operation cannot be performed because the specified VM is \
       protected by HA"
    () ;
  ...
  error Api_errors.illegal_vbd_device ["vbd"; "device"]
    ~doc:
      "The specified VBD device is not recognized: please use a non-negative \
       integer"
    () ;
  ...
  error Api_errors.domain_exists ["vm"; "domid"]
    ~doc:
      "The operation could not be performed because a domain still exists for \
       the specified VM."
    () ;
  ...
  error Api_errors.not_system_domain ["vm"]
    ~doc:
      "The given VM is not registered as a system domain. This operation can \
       only be performed on a registered system domain."
    () ;
  error Api_errors.vm_cannot_delete_default_template ["vm"]
    ~doc:"You cannot delete the specified default template." () ;
  error Api_errors.vm_bad_power_state
    ["vm"; "expected"; "actual"]
    ~doc:
  ...
  error Api_errors.host_not_enough_pcpus ["vcpus"; "pcpus"]
    ~doc:
      "The host does not have enough pCPUs to run the VM. It needs at least as \
       many as the VM has vCPUs."
    () ;
  ...
  error Api_errors.xen_vss_req_error_start_snapshot_set_failed
    ["vm"; "error_code"] ~doc:"An attempt to start a new VSS snapshot failed" () ;
  error Api_errors.xen_vss_req_error_adding_volume_to_snapset_failed
    ["vm"; "error_code"]
    ~doc:
  ...
  error Api_errors.xen_vss_req_error_creating_snapshot_xml_string
    ["vm"; "error_code"]
    ~doc:
  ...
  error Api_errors.vm_pv_drivers_in_use ["vm"] ~doc:"VM PV drivers still in use"
    () ;
  error Api_errors.vcpu_max_not_cores_per_socket_multiple
    ["vcpu_max"; "cores_per_socket"]
    ~doc:"VCPUs_max must be a multiple of cores-per-socket" () ;
  (* VM appliance errors *)
  error Api_errors.operation_partially_failed ["operation"]
    ~doc:
      "Some VMs belonging to the appliance threw an exception while carrying \
  ...
  error Api_errors.host_in_use ["host"; "type"; "ref"]
    ~doc:
      "This operation cannot be completed as the host is in use by (at least) \
       the object of type and ref echoed below."
    () ;
  error Api_errors.host_not_disabled []
    ~doc:
      "This operation cannot be performed because the host is not disabled. \
       Please disable the host and then try again."
    () ;
  ...
  error Api_errors.host_power_on_mode_disabled []
    ~doc:
      "This operation cannot be completed because the server power on mode is \
       disabled."
    () ;
  ...
  error Api_errors.host_has_no_management_ip []
    ~doc:
      "The server failed to acquire an IP address on its management interface \
       and therefore cannot contact the coordinator."
    () ;
  ...
  error Api_errors.host_broken []
    ~doc:
      "This server failed in the middle of an automatic failover operation and \
       needs to retry the failover action."
    () ;
  ...
  error Api_errors.interface_has_no_ip ["interface"]
    ...
    () ;
  error Api_errors.auth_already_enabled
    ["current auth_type"; "current service_name"]
  ...
  error Api_errors.pool_joining_host_cannot_be_master_of_other_hosts []
    ~doc:
      "The server joining the pool cannot already be a coordinator of another \
       pool."
    () ;
  ...
  error Api_errors.pool_joining_host_service_failed []
    ~doc:
      "There was an error connecting to the server. The service contacted \
  ...
  error Api_errors.pool_joining_host_must_have_same_product_version []
    ~doc:
      "The server joining the pool must have the same product version as the \
       pool coordinator."
    () ;
  error Api_errors.pool_joining_host_must_only_have_physical_pifs []
    ...
    () ;
  error Api_errors.pool_joining_host_management_vlan_does_not_match
    ["local"; "remote"]
  ...
  error Api_errors.pool_auth_already_enabled ["host"]
    ~doc:
      "External authentication is already enabled for at least one server in \
       this pool."
    () ;
  ...
  error Api_errors.pool_auth_disable_failed_wrong_credentials
    ["host"; "message"]
    ~doc:
      "External authentication has been disabled with errors: Some AD machine \
       accounts were not disabled on the AD server due to invalid credentials."
    () ;
  error Api_errors.pool_auth_disable_failed_permission_denied
    ["host"; "message"]
    ~doc:
      "External authentication has been disabled with errors: Your AD machine \
       account was not disabled on the AD server as permission was denied."
    () ;
  error Api_errors.pool_auth_disable_failed_invalid_account ["host"; "message"]
    ~doc:
      "External authentication has been disabled with errors: Some AD machine \
       accounts were not disabled on the AD server due to invalid account."
    () ;
  error Api_errors.pool_joining_host_must_have_same_api_version
    ["host_api_version"; "master_api_version"]
    ~doc:
      "The host joining the pool must have the same API version as the pool \
       coordinator."
    () ;
  error Api_errors.pool_joining_host_must_have_same_db_schema
    ["host_db_schema"; "master_db_schema"]
    ~doc:
      "The host joining the pool must have the same database schema as the \
       pool coordinator."
    () ;
  ...
  error Api_errors.subject_already_exists [] ~doc:"Subject already exists." () ;

  (* RBAC *)
  error Api_errors.role_not_found [] ~doc:"Role cannot be found." () ;
  error Api_errors.role_already_exists [] ~doc:"Role already exists." () ;
  ...
  error Api_errors.wlb_malformed_response
    ...
    ~doc:
      "WLB said something that the server wasn't expecting or didn't \
       understand. The method called on WLB, a diagnostic reason, and the \
       response from WLB are returned."
    () ;
  error Api_errors.wlb_xenserver_connection_refused []
    ~doc:
      "WLB reported that the server refused to let it connect (even though \
       we're connecting perfectly fine in the other direction)."
    () ;
  error Api_errors.wlb_xenserver_unknown_host []
    ~doc:
      "WLB reported that its configured server name for this server instance \
       failed to resolve in DNS."
    () ;
  ...
  error Api_errors.wlb_xenserver_authentication_failed []
    ~doc:
      "WLB reported that the server rejected its configured authentication \
       details."
    () ;
  error Api_errors.wlb_xenserver_malformed_response []
    ~doc:
      "WLB reported that the server said something to it that WLB wasn't \
       expecting or didn't understand."
    () ;
  error Api_errors.wlb_internal_error [] ~doc:"WLB reported an internal error."
    () ;
  error Api_errors.wlb_connection_reset []
    ~doc:"The connection to the WLB server was reset." () ;
  error Api_errors.wlb_url_invalid ["url"]
    ~doc:
      "The WLB URL is invalid. Ensure it is in the format: <ipaddress>:<port>. \
       The configured/given URL is returned."
    () ;

  (* Api_errors specific to running VMs on multiple hosts *)
  error Api_errors.vm_unsafe_boot ["vm"]
    ~doc:
      "You attempted an operation on a VM that was judged to be unsafe by the \
       server. This can happen if the VM would run on a CPU that has a \
       potentially incompatible set of feature flags to those the VM requires. \
  ...
  error Api_errors.vm_requires_sr ["vm"; "sr"]
    ~doc:
      "You attempted to run a VM on a host which doesn't have access to an SR \
       needed by the VM. The VM has at least one VBD attached to a VDI in the \
       SR."
    () ;
  ...
  error Api_errors.vm_requires_gpu ["vm"; "GPU_group"]
    ~doc:
      "You attempted to run a VM on a host which doesn't have a pGPU available \
       in the GPU group needed by the VM. The VM has a vGPU attached to this \
       GPU group."
    () ;
  error Api_errors.vm_requires_vgpu
    ["vm"; "GPU_group"; "vGPU_type"]
    ~doc:
      "You attempted to run a VM on a host on which the vGPU required by the \
       VM cannot be allocated on any pGPUs in the GPU_group needed by the VM."
    () ;
  ...
  error Api_errors.vm_host_incompatible_version ["host"; "vm"]
    ~doc:
      "This VM operation cannot be performed on an older-versioned host during \
       an upgrade."
    () ;
  error Api_errors.vm_host_incompatible_virtual_hardware_platform_version
    ["host"; "host_versions"; "vm"; "vm_version"]
    ~doc:
      "You attempted to run a VM on a host that cannot provide the VM's \
       required Virtual Hardware Platform version."
    () ;
  ...
  error Api_errors.host_cannot_attach_network ["host"; "network"]
    ~doc:
      "Server cannot attach network (in the case of NIC bonding, this may be \
       because attaching the network on this server would require other \
  ...
  error Api_errors.vm_migrate_failed
    ["vm"; "source"; "destination"; "msg"]
  ...
  error Api_errors.vm_crashed ["vm"] ~doc:"The VM crashed" () ;
  error Api_errors.vm_rebooted ["vm"] ~doc:"The VM unexpectedly rebooted" () ;
  error Api_errors.vm_halted ["vm"] ~doc:"The VM unexpectedly halted" () ;
  ...
  error
    ...
    ["vm"]
    ~doc:
      "You attempted to start a VM that's attached to more than one VDI with a \
       timeoffset marked as reset-on-boot."
    () ;
  ...
  error Api_errors.ballooning_timeout_before_migration ["vm"]
    ~doc:
      "Timeout trying to balloon down memory before VM migration. If the error \
       occurs repeatedly, consider increasing the memory-dynamic-min value."
    () ;
  error Api_errors.vm_requires_vusb ["vm"; "USB_group"]
    ~doc:
      "You attempted to run a VM on a host on which the VUSB required by the \
       VM cannot be allocated on any PUSBs in the USB_group needed by the VM."
    () ;

  (* Storage errors *)
  error Api_errors.sr_not_attached ["sr"] ~doc:"The SR is not attached." () ;
  error Api_errors.sr_attach_failed ["sr"] ~doc:"Attaching this SR failed." () ;
  error Api_errors.sr_backend_failure
    ["status"; "stdout"; "stderr"]
    ~doc:"There was an SR backend failure." () ;
  error Api_errors.sr_uuid_exists ["uuid"]
    ~doc:"An SR with that uuid already exists." () ;
  error Api_errors.sr_no_pbds ["sr"] ~doc:"The SR has no attached PBDs" () ;
  ...
  error Api_errors.sr_has_multiple_pbds ["PBD"]
    ~doc:
      "The SR.shared flag cannot be set to false while the SR remains \
  ...
  error Api_errors.vdi_not_sparse ["vdi"]
    ~doc:
      "The VDI is not stored using a sparse format. It is not possible to \
       query and manipulate only the changed blocks (or 'block differences' or \
       'disk deltas') between two VDIs. Please select a VDI which uses a \
  ...
  error Api_errors.vdi_is_not_iso ["vdi"; "type"]
    ~doc:
      "This operation can only be performed on CD VDIs (iso files or CDROM \
       drives)"
    () ;
  ...
  error Api_errors.vdi_not_available ["vdi"]
    ~doc:
      "This operation cannot be performed because this VDI could not be \
       properly attached to the VM."
    () ;
  ...
  error Api_errors.vdi_missing ["sr"; "vdi"]
    ~doc:
      "This operation cannot be performed because the specified VDI could not \
       be found on the storage substrate"
    () ;
  ...
  error Api_errors.vdi_cbt_enabled ["vdi"]
    ~doc:
      "The requested operation is not allowed for VDIs with CBT enabled or VMs \
       having such VDIs, and CBT is enabled for the specified VDI."
    () ;
  error Api_errors.vdi_no_cbt_metadata ["vdi"]
    ~doc:
      "The requested operation is not allowed because the specified VDI does \
       not have changed block tracking metadata."
    () ;
  ...
  error Api_errors.vdi_on_boot_mode_incompatible_with_operation []
    ~doc:
      "This operation is not permitted on VDIs in the 'on-boot=reset' mode, or \
       on VMs having such VDIs."
    () ;
  error Api_errors.cannot_create_state_file []
    ~doc:
      "An HA statefile could not be created, perhaps because no SR with the \
       appropriate capability was found."
    () ;
  error Api_errors.vif_not_in_map ["vif"]
    ~doc:
      "This VIF was not mapped to a destination Network in VM.migrate_send \
       operation"
    () ;

  error Api_errors.suspend_image_not_accessible ["vdi"]
    ~doc:
      "The suspend image of a checkpoint is not accessible from the host on \
  ...
  error Api_errors.sr_device_in_use []
    ~doc:
      "The SR operation cannot be performed because a device underlying the SR \
       is in use by the server."
    () ;
  error Api_errors.sr_not_sharable ["sr"; "host"]
    ~doc:
      "The PBD could not be plugged because the SR is in use by another host \
       and is not marked as sharable."
    () ;
  error Api_errors.sr_indestructible ["sr"]
    ~doc:
      "The SR could not be destroyed because the 'indestructible' flag was set \
       on it."
    () ;
  ...
  error Api_errors.sm_plugin_communication_failure ["sm"]
    ~doc:"The SM plug-in did not respond to a query." () ;
  ...
  error Api_errors.invalid_device ["device"] ~doc:"The device name is invalid"
    () ;
  ...
  error Api_errors.import_incompatible_version []
    ~doc:
      "The import failed because this export has been created by a different \
  ...
  error Api_errors.import_error_premature_eof []
    ~doc:
      "The VM could not be imported; the end of the file was reached \
       prematurely."
    () ;
  ...
  error Api_errors.import_error_failed_to_find_object ["id"]
    ~doc:
      "The VM could not be imported because a required object could not be \
       found."
    () ;
  error Api_errors.import_error_attached_disks_not_found []
    ...
    () ;
  error Api_errors.import_error_unexpected_file
    ["filename_expected"; "filename_found"]
    ~doc:
      "The VM could not be imported because the XVA file is invalid: an \
       unexpected file was encountered."
    () ;

  (* Restore errors *)
  error Api_errors.restore_incompatible_version []
    ~doc:
      "The restore could not be performed because this backup has been created \
       by a different (incompatible) product version"
    () ;
  error Api_errors.restore_target_missing_device ["device"]
    ~doc:
      "The restore could not be performed because a network interface is \
       missing"
    () ;
  error Api_errors.restore_target_mgmt_if_not_in_backup []
    ~doc:
      "The restore could not be performed because the server's current \
       management interface is not in the backup. The interfaces mentioned in \
       the backup are:"
    () ;

  error Api_errors.cannot_find_state_partition []
    ~doc:
      "This operation could not be performed because the state partition could \
       not be found"
    () ;
  ...
  error Api_errors.event_subscription_parse_failure ["subscription"]
    ~doc:
      "The server failed to parse your event subscription. Valid values \
       include: *, class-name, class-name/object-reference."
    () ;
  error Api_errors.event_from_token_parse_failure ["token"]
    ~doc:
      "The event.from token could not be parsed. Valid values include: '', and \
       a value returned from a previous event.from call."
    () ;
  ...
  error Api_errors.too_many_pending_tasks []
    ~doc:
      "The request was rejected because there are too many pending tasks on \
       the server."
    () ;
  ...
  error Api_errors.invalid_patch [] ~doc:"The uploaded patch file is invalid" () ;
  ...
  error Api_errors.cannot_find_patch []
    ~doc:
      "The requested update could not be found. This can occur when you \
       designate a new coordinator or xe patch-clean. Please upload the update \
       again."
    () ;
  ...
  error Api_errors.patch_apply_failed_backup_files_exist ["output"]
    ~doc:
      "The patch apply failed: there are backup files created while applying \
       patch. Please remove these backup files before applying patch again."
    () ;
  error Api_errors.patch_precheck_failed_unknown_error ["patch"; "info"]
    ~doc:
      "The patch pre-check stage failed with an unknown error. See attached \
       info for more details."
    () ;
  error Api_errors.patch_precheck_failed_prerequisite_missing
    ["patch"; "prerequisite_patch_uuid_list"]
    ~doc:"The patch pre-check stage failed: prerequisite patches are missing."
    () ;
  error Api_errors.patch_precheck_failed_wrong_server_version
    ["patch"; "found_version"; "required_version"]
    ~doc:
      "The patch pre-check stage failed: the server is of an incorrect version."
    () ;
  error Api_errors.patch_precheck_failed_wrong_server_build
    ["patch"; "found_build"; "required_build"]
    ~doc:
  ...
  error Api_errors.patch_precheck_failed_vm_running ["patch"]
    ~doc:
      "The patch pre-check stage failed: there are one or more VMs still \
       running on the server. All VMs must be suspended before the patch can \
       be applied."
    () ;
  error Api_errors.patch_precheck_failed_out_of_space
    ["patch"; "found_space"; "required_required"]
    ~doc:
  ...
  error Api_errors.update_precheck_failed_prerequisite_missing
    ["update"; "prerequisite_update"]
    ~doc:
      "The update pre-check stage failed: prerequisite update(s) are missing."
    () ;
  error Api_errors.update_precheck_failed_conflict_present
    ["update"; "conflict_update"]
    ~doc:"The update pre-check stage failed: conflicting update(s) are present."
    () ;
  error Api_errors.update_precheck_failed_wrong_server_version
    ["update"; "installed_version"; "required_version "]
    ~doc:
      "The update pre-check stage failed: the server is of an incorrect \
       version."
    () ;
  error Api_errors.update_precheck_failed_out_of_space
    ["update"; "available_space"; "required_space "]
    ~doc:
      "The update pre-check stage failed: the server does not have enough \
       space."
    () ;
  error Api_errors.update_precheck_failed_gpgkey_not_imported ["update"]
    ~doc:
      "The update pre-check stage failed: RPM package validation requires a \
       GPG key that is not present on the host."
    () ;
  ...
  error Api_errors.ha_failed_to_form_liveset []
    ~doc:
      "HA could not be enabled on the Pool because a liveset could not be \
       formed: check storage and network heartbeat paths."
    () ;
  error Api_errors.ha_heartbeat_daemon_startup_failed []
    ~doc:
      "The server could not join the liveset because the HA daemon failed to \
       start."
    () ;
  error Api_errors.ha_host_cannot_access_statefile []
    ~doc:
      "The server could not join the liveset because the HA daemon could not \
       access the heartbeat disk."
    () ;
  error Api_errors.ha_host_is_armed ["host"]
    ~doc:
      "The operation could not be performed while the server is still armed; \
  ...
  error Api_errors.ha_not_enabled []
    ~doc:
      "The operation could not be performed because HA is not enabled on the \
       Pool"
    () ;
  ...
  error Api_errors.ha_not_installed ["host"]
    ~doc:
      "The operation could not be performed because the HA software is not \
       installed on this server."
    () ;
  error Api_errors.ha_host_cannot_see_peers ["host"; "all"; "subset"]
    ~doc:
      "The operation failed because the HA software on the specified server \
       could not see a subset of other servers. Check your network \
       connectivity."
    () ;
  error Api_errors.ha_too_few_hosts []
    ~doc:
      "HA can only be enabled for 2 servers or more. Note that 2 servers \
       requires a pre-configured quorum tiebreak script."
    () ;
  error Api_errors.ha_should_be_fenced ["host"]
    ~doc:
      "Server cannot rejoin pool because it should have fenced (it is not in \
       the coordinator's partition)."
    () ;
  error Api_errors.ha_abort_new_master ["reason"]
    ~doc:
      "This server cannot accept the proposed new coordinator setting at this \
       time."
    () ;

  error Api_errors.ha_no_plan []
    ~doc:
      "Cannot find a plan for placement of VMs as there are no other servers \
       available."
    () ;
  ...
  error Api_errors.ha_pool_is_enabled_but_host_is_disabled []
    ~doc:
      "This server cannot join the pool because the pool has HA enabled but \
       this server has HA disabled."
    () ;
  error Api_errors.ha_constraint_violation_sr_not_shared ["SR"]
    ~doc:
      "This operation cannot be performed because the referenced SR is not \
       properly shared. The SR must both be marked as shared and a \
       currently_attached PBD must exist for each host."
    () ;
  error Api_errors.ha_constraint_violation_network_not_shared ["network"]
    ~doc:
      "This operation cannot be performed because the referenced network is \
       not properly shared. The network must either be entirely virtual or \
  ...
  error Api_errors.ha_operation_would_break_failover_plan []
    ~doc:
      "This operation cannot be performed because it would invalidate VM \
       ...
       restart protected VMs after a Host failure."
    () ;

  error Api_errors.ha_cannot_change_bond_status_of_mgmt_iface []
    ~doc:
      "This operation cannot be performed because creating or deleting a bond \
       involving the management interface is not allowed while HA is on. In \
       order to do that, disable HA, create or delete the bond then re-enable \
       HA."
    () ;
  ...
  error Api_errors.system_status_retrieval_failed ["reason"]
    ~doc:
      "Retrieving system status from the host failed. A diagnostic reason \
       suitable for support organisations is also returned."
    () ;
  ...
  error Api_errors.xapi_hook_failed
    ["hook_name"; "reason"; "stdout"; "exit_code"]
    ~doc:"3rd party xapi hook failed" () ;

  error Api_errors.xenapi_missing_plugin ["name"]
    ~doc:"The requested plug-in could not be found." () ;
  error Api_errors.xenapi_plugin_failure
    ["status"; "stdout"; "stderr"]
    ~doc:"There was a failure communicating with the plug-in." () ;

  error Api_errors.domain_builder_error
  ...
  error Api_errors.server_certificate_key_algorithm_not_supported
    ["algorithm_oid"] ~doc:"The provided key uses an unsupported algorithm." () ;
  ...
  error Api_errors.ssl_verify_error ["reason"]
    ~doc:
      "The remote system's SSL certificate failed to verify against our \
       certificate library."
    () ;
  ...
  error Api_errors.redo_log_is_enabled []
    ~doc:
      "The operation could not be performed because a redo log is enabled on \
       the Pool."
    () ;

  error Api_errors.vm_bios_strings_already_set []
    ~doc:
      "The BIOS strings for this VM have already been set and cannot be \
       changed."
    () ;
  ...
  error Api_errors.vdi_contains_metadata_of_this_pool ["vdi"; "pool"]
    ~doc:
      "The VDI could not be opened for metadata recovery as it contains the \
       current pool's metadata."
    () ;
  ...
  error Api_errors.could_not_import_database ["reason"]
    ~doc:
      "An error occurred while attempting to import a database from a metadata \
       VDI"
    () ;
  ...
  error Api_errors.vm_to_import_is_not_newer_version
    ["vm"; "existing_version"; "version_to_import"]
    ~doc:
      "The VM cannot be imported unforced because it is either the same \
       version or an older version of an existing VM."
    () ;

  error Api_errors.vm_call_plugin_rate_limit ["VM"; "interval"; "wait"]
    ~doc:
      "There is a minimal interval required between consecutive plug-in calls \
       made on the same VM, please wait before retry."
    () ;
  ...
  error Api_errors.too_many_vusbs ["number"] ~doc:"The VM has too many VUSBs."
    () ;
  ...
  error Api_errors.cluster_create_in_progress []
    ~doc:
      "The operation could not be performed because cluster creation is in \
       progress."
    () ;
  ...
  error Api_errors.clustering_enabled ["cluster_host"]
    ~doc:
      "An operation was attempted while clustering was enabled on the \
       cluster_host."
    () ;
  error Api_errors.clustering_disabled ["cluster_host"]
    ~doc:
      "An operation was attempted while clustering was disabled on the \
       cluster_host."
    () ;
  error Api_errors.cluster_does_not_have_one_node ["number_of_nodes"]
    ~doc:
      "An operation failed as it expected the cluster to have only one node \
       but found multiple cluster_hosts."
    () ;
  ...
  error Api_errors.cluster_force_destroy_failed ["cluster"]
    ~doc:
      "Force destroy failed on a Cluster_host while force destroying the \
       cluster."
    () ;
  ...
  error Api_errors.xen_incompatible []
    ~doc:
      "The current version of Xen or its control libraries is incompatible \
       with the Toolstack."
    () ;
  ...
  error Api_errors.configure_repositories_in_progress []
    ~doc:
      "The operation could not be performed because other repository(ies) \
       is(are) already being configured."
    () ;
  ...
  message
    ...
    ~doc:
      "Pool has become overcommitted: it can no longer guarantee to restart \
       protected VMs if the configured number of hosts fail."
    () ;
  message
    (fst Api_messages.ha_statefile_lost)
    ~doc:"Host lost access to HA storage heartbeat" () ;
  message
    (fst Api_messages.ha_heartbeat_approaching_timeout)
    ~doc:"HA network heartbeat almost timed-out" () ;
  message
    (fst Api_messages.ha_statefile_approaching_timeout)
    ~doc:"HA storage heartbeat almost timed-out" () ;
  message
    (fst Api_messages.ha_xapi_healthcheck_approaching_timeout)
    ~doc:"HA xapi healthcheck almost timed-out" () ;
  message
    (fst Api_messages.ha_network_bonding_error)
    ~doc:"HA network heartbeat interface bonding error" () ;
  message
    (fst Api_messages.vif_qos_failed)
    ~doc:"Applying QoS to VIF failed." () ;
  message
    (fst Api_messages.vbd_qos_failed)
    ~doc:"Applying QoS to VBD failed." () ;
  message
  ...
  message
    ...
    ~doc:"Host failed to attach one or more Storage Repositories." () ;
  ()
